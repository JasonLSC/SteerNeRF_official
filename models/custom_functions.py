...
class VolumeRenderer(torch.autograd.Function):
    ...
    def forward(ctx, sigmas, rgbs, deltas, ts, rays_a, T_threshold):
        ...
        #                             rays_a, T_threshold)
        total_samples, opacity, depth, rgb, ws = \
            vren.composite_train_feat_fw(sigmas, rgbs, deltas, ts,
                                    rays_a, T_threshold)
    ...
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_drgb, dL_dws):
        # print(dL_drgb.is_contiguous())
        # print(dL_ddepth.is_contiguous())
        # contiguous() bug
        # print(dL_drgb.shape)
        dL_drgb = dL_drgb.contiguous()
        ...
        #                             ctx.T_threshold)
        dL_dsigmas, dL_drgbs = \
            vren.composite_train_feat_bw(dL_dopacity, dL_ddepth, dL_drgb, dL_dws,
                                    sigmas, rgbs, ws, deltas, ts,
                                    rays_a,
                                    opacity, depth, rgb,
                                    ctx.T_threshold)

...
MAX_SAMPLES = 1024 # default: 1024
NEAR_DISTANCE = 0.01


def render(model, rays_o, rays_d, depth_proj=None, **kwargs):
    """
    Render rays by
    ...
    if depth_proj is not None:
        depth_proj = depth_proj.view(-1,1)
        mask = (depth_proj > 0).flatten()
        hits_t[mask,:,0] = torch.max( hits_t[mask,:,0], depth_proj[mask]-0.05 )
        hits_t[mask,:,1] = torch.min( hits_t[mask,:,1], depth_proj[mask]+0.05 )
        if False:
            import matplotlib.pyplot as plt
            plt.imshow(hits_t[...,0].reshape(200,200).detach().cpu().numpy())
            plt.show()
            plt.imshow(hits_t[...,1].reshape(200,200).detach().cpu().numpy())
            plt.show()
...
def __render_rays_test(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    rgb = torch.zeros(N_rays, 6, device=device)


    samples = total_samples = 0
    alive_indices = torch.arange(N_rays, device=device)
    alive_mask = torch.zeros_like(alive_indices, device=device)
    ...
    min_samples = 64 if exp_step_factor==0 else 4
    # min_samples = 16 # when "TanksAndTemple", we should not use so many samples
    #                   # leading to OOM.

    # pdb.set_trace()
    n_loop = 0
    kwargs['T_threshold'] = 1e-2
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        if False:
            import matplotlib.pyplot as plt
            import numpy as np
            A = np.zeros((200,200)).flatten()
            A[alive_indices.detach().cpu().numpy()]=N_eff_samples.detach().float().cpu().numpy()
            #plt.imshow(N_eff_samples.reshape(200,200).detach().float().cpu().numpy())
        ...
        rgbs = torch.zeros(len(xyzs), 6, device=device)

        if kwargs.get("output_feature", False):
            pdb.set_trace()
            sigmas, rgbs, feats = model(xyzs, dirs, **kwargs)
            _sigmas, _rgbs, _feats = model(xyzs[valid_mask], dirs[valid_mask], **kwargs)
            sigmas[valid_mask], rgbs[valid_mask], feats[valid_mask] = _sigmas.float(), _rgbs.float(), _feats.float()
        ...
        vren.composite_test_feat_fw(
            sigmas, rgbs, deltas, ts,
            hits_t[:, 0], alive_indices, kwargs.get('T_threshold', 1e-4), # 1e-4
            N_eff_samples, opacity, depth, rgb)
        alive_indices = alive_indices[alive_indices>=0] # remove converged rays
        if False:
            import matplotlib.pyplot as plt
            alive_mask = torch.zeros_like(alive_mask)
            alive_mask[alive_indices] = 1.
            plt.imshow(alive_mask.reshape(200,200).detach().cpu().numpy())
            plt.show()
    ...
    results['feat'] += rgb_bg*rearrange(1-opacity, 'n -> n 1')
    return results
...
def __render_rays_train(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    results['opacity'], results['depth'], results['rgb'], results['ws']) = \
        VolumeRenderer.apply(sigmas, rgbs.contiguous(), results['deltas'], results['ts'],
                             rays_a, kwargs.get('T_threshold', 1e-2))
    results['rays_a'] = rays_a

    results['feat'] = results['rgb'][..., 3:6]
    results['rgb'] = results['rgb'][..., 0:3]

...
std::vector<torch::Tensor> composite_train_feat_fw(
    const torch::Tensor sigmas,
    const torch::Tensor rgbs,
    const torch::Tensor deltas,
    const torch::Tensor ts,
    ...
    return composite_train_feat_fw_cu(
                sigmas, rgbs, deltas, ts,
                rays_a, opacity_threshold);
}


std::vector<torch::Tensor> composite_train_feat_bw(
    const torch::Tensor dL_dopacity,
    const torch::Tensor dL_ddepth,
    const torch::Tensor dL_drgb,
    const torch::Tensor dL_dws,
    ...
    return composite_train_feat_bw_cu(
                dL_dopacity, dL_ddepth, dL_drgb, dL_dws,
                sigmas, rgbs, ws, deltas, ts, rays_a,
                opacity, depth, rgb, opacity_threshold);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("composite_train_feat_fw", &composite_train_feat_fw);
    m.def("composite_train_feat_bw", &composite_train_feat_bw);
    m.def("composite_test_feat_fw", &composite_test_feat_fw);

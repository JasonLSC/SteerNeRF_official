...
__global__ void composite_train_feat_fw_kernel(
    ...
    while (samples < N_samples) {
        ...
        rgb[ray_idx][3] += w*rgbs[s][3];
        rgb[ray_idx][4] += w*rgbs[s][4];
        rgb[ray_idx][5] += w*rgbs[s][5];
...
std::vector<torch::Tensor> composite_train_feat_fw_cu(
    ...
    auto rgb = torch::zeros({N_rays, 6}, sigmas.options());
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "composite_train_feat_fw_cu",
    ([&] {
        composite_train_feat_fw_kernel<scalar_t><<<blocks, threads>>>(
...
template <typename scalar_t>
__global__ void composite_train_feat_bw_kernel(
    ...
    scalar_t F0 = rgb[ray_idx][3], F1 = rgb[ray_idx][4], F2 = rgb[ray_idx][5];
    scalar_t O = opacity[ray_idx], D = depth[ray_idx];
    scalar_t T = 1.0f, r = 0.0f, g = 0.0f, b = 0.0f, d = 0.0f;
    scalar_t f0 = 0.0f, f1 = 0.0f, f2 = 0.0f;
    ...
    while (samples < N_samples) {
        ...
        f0 += w*rgbs[s][3]; f1 += w*rgbs[s][4]; f2 += w*rgbs[s][5];
        d += w*ts[s];
        T *= 1.0f-a;
        ...
        dL_drgbs[s][3] = dL_drgb[ray_idx][3]*w;
        dL_drgbs[s][4] = dL_drgb[ray_idx][4]*w;
        dL_drgbs[s][5] = dL_drgb[ray_idx][5]*w;
        ...
        dL_dsigmas[s] = deltas[s] * (
            ...
            dL_drgb[ray_idx][3]*(rgbs[s][3]*T-(F0-f0)) +
            dL_drgb[ray_idx][4]*(rgbs[s][4]*T-(F1-f1)) +
            dL_drgb[ray_idx][5]*(rgbs[s][5]*T-(F2-f2)) + // gradients from feat
...
std::vector<torch::Tensor> composite_train_feat_bw_cu(
    ...
    auto dL_drgbs = torch::zeros({N, 6}, sigmas.options());

    auto dL_dws_times_ws = dL_dws * ws; // auxiliary input

    const int threads = 256, blocks = (N_rays+threads-1)/threads;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "composite_train_feat_bw_cu",
    ([&] {
        composite_train_feat_bw_kernel<scalar_t><<<blocks, threads>>>(
...
template <typename scalar_t>
__global__ void composite_test_feat_fw_kernel(
    ...
    while (s < N_eff_samples[n]) {
        ...
        rgb[r][3] += w*rgbs[n][s][3];
        rgb[r][4] += w*rgbs[n][s][4];
        rgb[r][5] += w*rgbs[n][s][5];
        depth[r] += w*ts[n][s];
        opacity[r] += w;
        T *= 1.0f-a;
...
void composite_test_feat_fw_cu(
    const torch::Tensor sigmas,
    const torch::Tensor rgbs,
    const torch::Tensor deltas,
    const torch::Tensor ts,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "composite_test_feat_fw_cu",
    ([&] {
        composite_test_feat_fw_kernel<scalar_t><<<blocks, threads>>>(

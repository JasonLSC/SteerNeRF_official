...
class NGP(nn.Module):
    def __init__(self, scale, rgb_act='Sigmoid', T=19):
        ...
        self.rgb_net = \
            tcnn.Network(
                n_input_dims=32, n_output_dims=6, # rgb+feat
                network_config={
                    "otype": "FullyFusedMLP",
                    "activation": "ReLU",
    ...
    def forward(self, x, d, **kwargs):
        ...
        if kwargs.get('output_feature', False): # output feature for better SR
            return sigmas, rgbs, h
        else:
            return sigmas, rgbs
    ...
    def mark_invisible_cells(self, K, poses, img_wh, chunk=64**3):
        ...
        if len(poses.shape)==4: # (N_cams, frame_num, 3, 4)
            #poses = rearrange(poses, 'n f a b -> (n f) a b')
            poses = poses[:,0]
            if poses.shape[0]>100: # avoid out of memory
                poses = poses[::10]
